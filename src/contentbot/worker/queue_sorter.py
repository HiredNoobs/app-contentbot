...
def plan_moves(current_order: List[int], desired_order: List[int]) -> List[Dict[str, str | int]]:
    """
    ...
    stable = _longest_increasing_subsequence([desired_positions[uid] for uid in current_order])
...
class QueueSorter:
    ...
    async def sort_queue(self, playlist: List[Dict]) -> Dict:
        ...
        return {
            "type": "sort_queue",
            "moves": plan_moves(current_order, desired_order),
    ...
    async def _get_publish_time(self, item: Dict) -> Optional[datetime]:
        ...
        if cached:
            try:
                return as_utc(datetime.fromisoformat(cached))
            except ValueError:
                logger.warning("Ignoring invalid cached publish time for %s: %s", video_id, cached)

        published = await fetch_publish_time(video_id)
        if published:
...
async def fetch_publish_time(video_id: str) -> Optional[datetime]:
    """
    ...
    for pattern in PUBLISH_TIME_PATTERNS:
        ...
        raw = match.group(1)
        try:
            published = as_utc(datetime.fromisoformat(raw))
        except ValueError:
            continue
